...
%             @PERCENT1@NUM1) -- expected to be negligible, re-run for consistency. 2026-09-24: the
%             strong/weak labels are now cut per essay set with a gap (they were adjacent score points
%             in 6 of 8 sets), which changes the balanced ASAP pool and every ASAP item.
...
%             Hiring's probe/eval split is also grouped by biography since 2026-09-23.
%             And hiring's `qualified` label is re-assigned after the age rules on a profession-capped
%             pool (2026-09-23), which changes the hiring population and every label.
...
%             corrected from white to Black in the decision frame; (f) every education design and
%             A2 now read one shared essay pool, and A2 is framed in the A1 header with the
%             assignment and inserts at paragraph ends too (standpoint numbers); (g) the pool is
%             balanced strong/weak within prompt, A2's demographic axes are per-attribute pairs
%             cut from the A1 factorial with the A1 income wording, and the probe/eval split is
%             grouped by essay; and
%             (d) the rendered header now carries the assignment and the body keeps its paragraph
%             breaks, which changes every education item; (h) 2026-09-24: A2's conclusion sentence is
%             its own closing paragraph, and the shared pool drops essays ending in a sign-off or a
%             fragment (1,422 -> 1,364 essays). The ASAP education numbers are not marked
%             \P individually: every one of them already carries \ddag, so none is quotable either way.
% 2026-09-24: \crossAccBaseline* and \crossN* are retired with direct-form cross-influence (the runner
% and its exporter block were deleted); their \markstale lines went with them, and no document uses
% them any more. They leave numbers.tex at the next export.
% ALL marks, 2026-09-23: the capped eval split used to keep one factorial cell and one template per
% axis (credit, hiring, education); it now covers every cell x template, so every eval number
% changes on re-run (see the working notes' audit entry).
