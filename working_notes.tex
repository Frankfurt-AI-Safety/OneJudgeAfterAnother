...
intersection (the pregnancy window); \textbf{education (grading)} anchors instead on \textbf{sex $+$
ethnicity (name-proxy) $+$ disability}, since age and family/parental status are unnatural on a student
assignment. All are drawn from the same scope below.
\begin{itemize}
...
\end{itemize}
\subsection*{\today}
\todo{Move settled results up into progress\_*.tex, then into sections/. Build the education substrate
(edu\_ingest/edu\_render/edu\_dataset $+$ domain registry entry $+$ ethnicity/disability markers).}
