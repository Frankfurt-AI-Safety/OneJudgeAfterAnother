...
\begin{enumerate}
  ...
  \item \textbf{Medium.} \emph{Open.}
        ...
        differ by $\approx$14 points in good-credit rate and in rendered fields (dependents, checking). Several
        ...
        ceiling 0.79). ASAP's ``strong''/``weak'' are adjacent score points in 6 of 8 essay sets, so ``drop the
        middle'' is false there and ASAP is not comparable with PERSUADE. The hiring age rules leave a junior,
        skewed profession mix (nurses 4.8\% $\to$ 1.8\%, professors $\times$1.41). The planned real-demographic
        checks are confounded with quality (5 strong essays by real ELL writers).
  \item \textbf{Low.} Cross-influence pairs mostly mix prompts (39/300 share one); A2 insertion behaves slightly
        differently on weak essays (more unfinished last sentences); corpus anonymisation tokens are uneven
        by class; kept real demographics are confounded with profession/label; suspected crash in
        \texttt{run\_crossinfluence.py --encoding proxy} without \texttt{--axes} on credit's
        \texttt{marital\_status}.
...
\end{enumerate}
Items 1--3 are resolved. Still open (item 4): the credit real-field quality
confound, ASAP's adjacent strong/weak cut-offs, and conditioning the real-demographic checks on quality. The audit scripts and manifests are in the
