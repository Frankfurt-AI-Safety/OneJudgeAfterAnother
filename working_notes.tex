...
Three checks on whether our \emph{instruments} measure what we claim, to be run as one block. These bear on
how much of the A1 marker evidence survives, so they are scheduled ahead of further breadth.
\begin{enumerate}
  ...
  \item \textbf{Bias-in-Bios real CVs} to replace the fully synthetic hiring substrate, which currently
        carries the erasure, reasoning, and decision arms --- i.e.\ our most load-bearing arms sit on the
        least externally valid data.
...
\begin{itemize}
  \item \textbf{Why workshop first.} The write-up is explicitly in progress and the headline scaling results
        depend on compute landing. A workshop paper puts the reward-vs-representation gap and the
        standpoint-credibility effect in front of exactly the right community now, builds visibility while
        the grant applications are pending, and de-risks the later archival submission.
  \item \textbf{September is the decision point.} ICLR 2027 (2026-09-24) and the likely SoLaR / NeurIPS
        workshop deadlines both fall around September 2026 --- which is also when the rented compute is
        expected. Whether the scaling runs land by then determines archival-now vs.\ workshop-now.
